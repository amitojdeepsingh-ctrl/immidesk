...
interface SerializedClient {
  ...
  familyIntake: FamilyIntakeItem[];
}

export interface FamilyIntakeItem {
  id: string;
...
function applicantRoleLabel(label: string): string {
...
function intakeName(filledData: Record<string, unknown>): string {
  const meta = (filledData?._meta ?? {}) as Record<string, unknown>;
  const fromMeta = `${meta.firstName ?? ""} ${meta.lastName ?? ""}`.trim();
  if (fromMeta) return fromMeta;
  return `${filledData?.givenNames ?? ""} ${filledData?.surname ?? ""}`.trim();
}

function intakeWillApply(filledData: Record<string, unknown>): boolean {
  const meta = (filledData?._meta ?? {}) as Record<string, unknown>;
  return typeof meta.willApply === "boolean" ? meta.willApply : true;
}
...
export function ClientDetailView({ client }: ClientDetailViewProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Family & Intake — per-applicant PIS submissions */}
      <section className="rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        <div className="mb-3 flex items-center justify-between">
          ...
          {client.familyIntake.length > 0 && (
            <span className="text-xs text-zinc-400">
              {client.familyIntake.length} applicant
              {client.familyIntake.length !== 1 ? "s" : ""}
            </span>
          )}
        </div>
        ...
        {client.familyIntake.length === 0 ? (
          <p className="text-sm text-zinc-400 dark:text-zinc-500">
            No intake submissions yet. Share the client portal link to collect
                  ...
                  >
                    <td className="px-4 py-2.5">
                      <span className="inline-flex items-center rounded-full bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-600 dark:bg-blue-950/40 dark:text-blue-400">
                        {applicantRoleLabel(fi.applicantLabel)}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-2.5">
                      {intakeWillApply(fi.filledData) ? (
                        <span className="inline-flex items-center rounded-full bg-emerald-50 px-2 py-0.5 text-xs font-medium text-emerald-600 dark:bg-emerald-950/40 dark:text-emerald-400">
                          Yes
                        </span>
                      ) : (
